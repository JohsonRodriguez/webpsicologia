...
export type ActaTutoriaPdfData = {
  alumnoNombre: string;
  alumnoCodigo: string;
  fecha: string;
  hora: string;
...
export function ActaTutoriaPdfDocument({ data }: { data: ActaTutoriaPdfData }) {
  const firmaTutor = data.firmas.find((f) => f.firmanteTipo === "tutor");
  const firmaPadre = data.firmas.find((f) => f.firmanteTipo === "padre");

  return (
    <Document title={`Acta de tutoría — ${data.alumnoNombre}`}>
      <Page size="A4" style={styles.page}>
        <View style={styles.headerRow}>
          <View>
            <Text style={styles.eyebrow}>DEPARTAMENTO PSICOPEDAGÓGICO · COLEGIO LORD BYRON</Text>
            <Text style={styles.title}>Acta de reunión de tutoría con padres de familia</Text>
        ...
        <View style={styles.row}>
          ...
          <View style={styles.field}>
            ...
            <Text style={styles.fieldValue}>{data.tutorNombre}</Text>
          </View>
        </View>
        <View style={styles.field}>
        ...
        <View style={[styles.section, styles.twoCol]}>
          <View style={styles.col}>
            <Text style={styles.sectionTitle}>Acuerdos y compromisos del tutor</Text>
            <Text style={styles.paragraph}>{data.acuerdosTutor}</Text>
          </View>
          <View style={styles.col}>
        ...
        <View style={styles.signaturesRow}>
          <View style={styles.signatureBox}>
            {/* eslint-disable-next-line jsx-a11y/alt-text -- Image de @react-pdf/renderer, no soporta alt */}
            {firmaTutor && <Image src={firmaTutor.firmaData} style={styles.signatureImg} />}
            <Text style={styles.signatureName}>{data.tutorNombre}</Text>
            <Text style={styles.signatureMeta}>
              Tutor ·{" "}
              {firmaTutor ? new Date(firmaTutor.fechaHora).toLocaleString("es-PE", { dateStyle: "medium", timeStyle: "short" }) : "Firma no registrada"}
            </Text>
          </View>
          <View style={styles.signatureBox}>
            {/* eslint-disable-next-line jsx-a11y/alt-text -- Image de @react-pdf/renderer, no soporta alt */}
