...
import { crearActaTutoria, type EstadoAccion } from "../actions";
...
export function ActaTutoriaForm({
  alumnos,
  tieneFirmaGuardada,
}: {
  alumnos: { alumnoId: string; nombre: string; codigo: string }[];
  tieneFirmaGuardada: boolean;
}) {
  const router = useRouter();
  const [state, formAction, pending] = useActionState(crearActaTutoria, initialState);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-5 p-5">
        <div className="flex flex-col gap-1.5">
          ...
          <SimpleSelect
            ...
            placeholder={alumnos.length ? "Selecciona…" : "No hay alumnos en tu aula"}
        ...
        <div className="flex flex-col gap-1.5">
          <Label>Acuerdos y compromisos del tutor</Label>
          <Textarea name="acuerdos_tutor" required className="min-h-28 w-full" />
        </div>
        <div className="flex flex-col gap-1.5">
      ...
      <div className="flex justify-end gap-2 border-t border-border p-4">
        <Button type="button" variant="outline" onClick={() => router.push("/tutoria/actas")}>
          Cancelar
        </Button>
        <Button type="submit" disabled={pending || alumnos.length === 0}>
